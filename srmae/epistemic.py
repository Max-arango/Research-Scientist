...
def transition(obj: dict, to: str, *, evidence: list, by: str, ts: str,
               store=None) -> dict:
    ...
    If `store` is given, the new version is persisted (append-only) so metrics
    and provenance see the change. The same in-memory `obj` is returned for
    convenience; callers that pass `store` should read subsequent state from
    `store.get(obj["id"])` to avoid working on a stale version.
    """
