...
def compute(store) -> dict:
    ...
    # 5. falsification_rate: falsified (claims OR hypotheses) / (claims + hypotheses).
    # The adversary may falsify a claim directly without a hypothesis on file.
    bearing = hyps + claims
    falsified = [o for o in bearing if o.get("state") == "FALSIFIED"]
    falsification_rate = (len(falsified) / len(bearing)) if bearing else None
